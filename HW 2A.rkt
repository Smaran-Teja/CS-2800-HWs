...
(define (implies P Q) (or (not P) Q)) ; using a simpler logic for implies than the one provided in write-up

;; (P /\ Q) \/  ~(R /\ S)
(define (p2 P Q R S) (or (and P Q) (not (and R S))))

;; P -> ~Q
(define (p3 P Q) (implies P (not Q)))

;; ~(P /\ Q) = ~ P \/ ~Q
(define (p4 P Q) (boolean=? (not (and P Q)) (or (not P) (not Q))))
;; part p1
...
;; ~(P /\ Q) = ~P \/ ~Q
(define (p5 p q) (boolean=? (not (and p q)) (or (not p) (not q))))
...
;; ~(P \/ Q) = ~P /\ ~Q
(define (p6 p q) (boolean=? (not (or p q)) (and (not p) (not q))))
