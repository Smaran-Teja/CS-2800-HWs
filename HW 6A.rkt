...
(define (simple-const-fold p)
  (cond [(< (length p) 3) p]
        [(and (add? (third p))
              (push? (second p))
              (push? (first p))) (cons (make-push (+ (push-num (second p))
                                                     (push-num (first p))))
...
(define (simple-const-fold-prop l) (simple-stack-verify l (simple-const-fold l)))
...
(define (stack-verify env p1 p2) (local [(define p1-out (eval env '() p1))
                                         (define p2-out (eval env '() p2))]
    (equal? p1-out p2-out)))
...
(define (const-fold p) (cond [(< (length p) 3) p]
                             [(and (add? (third p))
                                   (push? (second p))
                                   (push? (first p))) (cons (make-push (+ (push-num (second p))
                                                                          (push-num (first p))))
...
(define (const-fold-prop l) (stack-verify '() l (const-fold l)))
...
(define (map-check loi f) (cond [(empty? loi) #t]
                                [else (and (= (f (first loi)) (first (map f loi)))
                                           (map-check (rest loi) f))]))
